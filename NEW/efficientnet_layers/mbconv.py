...
class MBConv(nn.Module):
    ...
    def __init__(self, c, sd_prob=0.0):
        ...
        if c.expand_ratio == 1:
            ...
            block.append(('fused_point_wise', PointwiseConvolution(inter_channel, c.out_ch, c.norm_layer, nn.Identity)))

        else:
            # block.append(('linear_bottleneck', ConvBnAct(c.in_ch, inter_channel, 1, 1, 1, c.norm_layer, c.act)))
            block.append(('point_wise', PointwiseConvolution(inter_channel, c.out_ch, c.norm_layer, c.act)))

            # block.append(('depth_wise', ConvBnAct(inter_channel, inter_channel, c.kernel, c.stride, inter_channel, c.norm_layer, c.act)))
            block.append(('depth_wise', DepthwiseConv(inter_channel, inter_channel, c.kernel, c.stride, inter_channel, c.norm_layer, c.act)))

            block.append(('se', SqueezeExcite(inter_channel, 4 * c.expand_ratio)))
            # block.append(('point_wise', ConvBnAct(inter_channel, c.out_ch, 1, 1, 1, c.norm_layer, nn.Identity)))
            block.append(('point_wise', PointwiseConvolution(inter_channel, c.out_ch, c.norm_layer, nn.Identity)))
